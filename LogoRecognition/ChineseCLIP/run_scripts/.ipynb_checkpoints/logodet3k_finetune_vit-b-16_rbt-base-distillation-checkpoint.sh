...
export MASTER_ADDR=125.227.53.125
...
# data options
train_data=${DATAPATH}/datasets/LogoDet-3K/lmdb/train
val_data=${DATAPATH}/datasets/LogoDet-3K/lmdb/valid # if val_data is not specified, the validation will be automatically disabled
...
name=logodet3k_finetune_vit-b-16_roberta-base_bs128_8gpu
...
python3 -m torch.distributed.launch --use_env --nproc_per_node=${GPUS_PER_NODE} --nnodes=${WORKER_CNT} --node_rank=${RANK} \
          ...
          --distllation \
          --teacher-model-name damo/multi-modal_team-vit-large-patch14_multi-modal-similarity
