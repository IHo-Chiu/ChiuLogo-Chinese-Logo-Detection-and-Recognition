...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root: str, charset: str, max_label_len: int, min_image_dim: int = 0,
        ...
        half2full = {'1':'１','2':'２','3':'３','4':'４','5':'５','6':'６','7':'７','8':'８','9':'９','0':'０',
        ...
        self.replace_char_list = {'é':'e', '´':'\'', 'á':'a', 'ü':'u', '：':':', '（':'(', '）':')', '—':'-', '！':'!', '▪': '·', '｜':'|', '丨':'|', '丶':'、', '”':'"', '；':';', '…':'...', '＆':'&', '•':'·', '′':'\'', '“':'"', '’':'\'', '【':'[', '】':']', 'ā':'a', '？':'?', 'í':'i', '℃':'°C', 'è':'e', '￥':'¥', 'ã':'a', '★':'·', 'ǎ':'a', '`':'\'', 'ē':'e', 'ұ':'¥', '∣':'|', '．':'·', '﹢':'+', '‘':'\'', 'ⅲ':'iii', '″':'"', '❝':'"', '❞':'"', '☰':'≡', '〈':'<', '〉':'>', 'ˋ':'\''}

        self.replace_char_list.update(full2half)
        self.test_list = {}
        self.num_samples = self._preprocess_labels(charset, remove_whitespace, normalize_unicode,
                                                   max_label_len, min_image_dim)

        self.test_list = [c for c in self.test_list.keys()]
        # print(''.join(self.test_list))
    ...
    def _preprocess_labels(self, charset, remove_whitespace, normalize_unicode, max_label_len, min_image_dim):
        ...
        with self._create_env() as env, env.begin() as txn:
            ...
            for index in range(num_samples):
                ...
                for c in self.replace_char_list.keys():
                    label = label.replace(c, self.replace_char_list[c])
    ...
    def __getitem__(self, index):
        ...
        # f = open("str_data_valid.txt", "a")
        # f.write(f"{label}\t{base64_str}\n")
        # f.close()
