...
def main():
    ...
    parser.add_argument('--chi', action='store_true', default=False, help='Evaluate on chinese benchmark datasets')
    ...
    with open('charset.txt','r') as f:
        desired_chars = f.read()
        desired_chars = desired_chars.replace('\n', '')
        ...
        for i in range(len(desired_chars)):
            if desired_chars[i] in ch_dict.keys():
                continue
            else:
                ch_dict[desired_chars[i]] = cnt
                cnt = cnt+1

                charset_test+= desired_chars[i]
    ...
    if args.chi:
        result_groups.update({'Chinese': SceneTextDataModule.TEST_CHINESE})
